...
- **`stream_events` v3 confirmed available**: Upgraded to deepagents 0.6.2 (all 102 tests pass). `agent.stream_events(input, version="v3")` is available and exposes `stream.subagents`, `subagent.name`, `subagent.tool_calls`, `tool_call.output_deltas`, `tool_call.output`, `tool_call.error` as per docs. Implementation uses this API.
